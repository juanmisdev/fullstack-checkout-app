// Receipt Printer — adapted from https://www.dqnamo.com/experiments/receipt-printer
// Original by dqnamo (interface.london). Adapted for this checkout flow:
// uses lucide-react icons, self-contained tokens, receipt data props.
...
const machineClassName =
  "relative isolate w-full overflow-hidden rounded-[var(--printer-radius)] border border-zinc-300 bg-zinc-200/70 p-[var(--printer-inset)] pb-8 shadow-[0_20px_36px_-20px_rgba(0,0,0,0.35),0_6px_14px_-8px_rgba(0,0,0,0.2),inset_0_1px_0_rgba(255,255,255,0.5),inset_0_-1px_0_rgba(0,0,0,0.2)] [--printer-inner-radius:calc(var(--printer-radius)_-_var(--printer-inset))] [--printer-inset:0.75rem] [--printer-radius:1.5rem]";
...
function ReceiptPrinterMachine({
  ...
  return (
    <div className={cn(machineClassName, className)} {...props}>
      ...
      <div
        aria-hidden="true"
        className="absolute inset-x-6 bottom-[var(--printer-inset)] z-40 h-2 rounded-[0.25rem] border border-zinc-600 bg-zinc-600 shadow-inner"
      />
    </div>
  );
}
...
function ReceiptPrinterScreen({
  ...
  return (
    <div
      className={cn(
        "relative z-10 isolate overflow-hidden rounded-[var(--printer-inner-radius)] border border-zinc-800 bg-zinc-900 p-4 text-zinc-50 shadow-inner after:pointer-events-none after:absolute after:inset-0 after:z-20 after:rounded-[inherit] after:shadow-[inset_0_0_24px_4px_rgba(0,0,0,0.5)] after:content-['']",
        className,
      )}
      {...props}
    >
...
function ReceiptPrinterPaper({
  ...
  return (
    <article
      className={cn(
        "relative z-10 min-h-80 bg-white px-6 pt-7 pb-8 font-mono text-zinc-900",
        className,
      )}
      ...
      <motion.div
        ...
        transition={{
          ...
          transform: {
            ...
            ease: shouldUseSteppedFeed ? "linear" : (easeInOut as never),
